...
(defun meet-and-greet-active-chase (c)
  (let* ((stream-x (open  "/tmp/position-x.dat1" :direction :output :if-exists :supersede))
         (stream-y (open "/tmp/position-y.dat1" :direction :output :if-exists :supersede))
         (stream-t-x (open "/tmp/target-x.dat1" :direction :output :if-exists :supersede))
         (stream-t-y (open "/tmp/target-y.dat1" :direction :output :if-exists :supersede)))
    (unwind-protect
               ...
               (if (first-iteration-p)
                   ...
                   (progn (for velocity = (2d-vector-- position p-position))
                          ...
                          (for thrust = (2d-vector-+ (2d-vector-* 4/5 q)
                                                     (2d-vector-* 2/5 p)))
                          (setf (control-structure-v-x c) (* 0.001 (2d-vector-x thrust))
